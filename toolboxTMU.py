...
def dataParser(exhibitStat, getTemp, getElect1, getElect2, getElect3, getH2, getMoist, dataLen, CTratio, PTratio):
    ...
    try:
        ...
        importKWH = (unsignedInt32Handler(getElect1.registers[29:31]))/10 #kWhImport
        exportKWH = (unsignedInt32Handler(getElect1.registers[31:33]))/10 #kWhExport
        importKVARH = (unsignedInt32Handler(getElect1.registers[33:35]))/10 #kVARhImport
        exportKVARH = (unsignedInt32Handler(getElect1.registers[35:]))/10 #kVARhExport
        outputData[34] = importKWH - exportKWH
        outputData[35] = importKVARH - exportKVARH
...
def HVParser(inputData, inputDataLVX, fixedTapRatio):
    outputData = [0]*39
    try:
        for i in range(0, 3):
            outputData[i] = fixedTapRatio * ((inputData[i] + inputDataLVX[i]) / 2) #Voltage Phase Neutral
            outputData[i + 3] = outputData[i] * math.sqrt(3) #Voltage Phase Phase
            outputData[i + 10] = inputData[i + 17] + inputDataLVX[i + 17] #P
            outputData[i + 14] = inputData[i + 21] + inputDataLVX[i + 21] #Q
            outputData[i + 18] = math.sqrt(math.pow(outputData[i + 10], 2) + math.pow(outputData[i + 14], 2)) #S
            outputData[i + 6] = (outputData[i + 18]) / (math.sqrt(3) * outputData[i + 3]) #Current
            outputData[i + 22] =  outputData[i + 10] / outputData[i + 18] #PF
        outputData[26] = inputData[34] + inputDataLVX[34] #kWh net
        outputData[27] = inputData[35] + inputDataLVX[35] #kVARh net
        outputData[13] = (outputData[29] + outputData[30] + outputData[31]) #Psig
        outputData[17] = (outputData[29] + outputData[30] + outputData[31]) #Qsig
        outputData[21] = (outputData[29] + outputData[30] + outputData[31]) #Ssig
        outputData[25] = (outputData[29] + outputData[30] + outputData[31])/3 #Average PF
        outputData[9] = (outputData[6] + outputData[7] + outputData[8])/3 #Average Current
    except:
        pass
    return outputData
