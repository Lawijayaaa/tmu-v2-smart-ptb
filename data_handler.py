...
from pymodbus.client import ModbusSerialClient
from toolboxTMU import parameter, sqlLibrary
from toolboxTMU import find_tap, initParameter, dataParser, harmonicParser, convertBinList, LVXParser, HVParser
...
engineName = "Trafo 30-PT-202"
teleURL = 'http://192.168.8.8:1444/api/transformer/sendNotificationToTelegramGroup'
progStat = True
debugMsg = False
infoMsg = True
dryType = False
...
def main():
    ...
    sheetName = ["Harmonic_phR", "Harmonic_phS", "Harmonic_phT", "LV_X", "HV"]
    ...
    while progStat:
        ...
        time.sleep(0.2)
        getElect1 = client.read_holding_registers(0, 37, slave = 2)
        time.sleep(0.2)
        ...
        time.sleep(0.2)
        getTempLVX = client.read_holding_registers(7, 3, slave = 3)
        time.sleep(0.2)
        getElect1LVX = client.read_holding_registers(0, 37, slave = 6)
        time.sleep(0.2)
        getElect2LVX = client.read_holding_registers(46, 5, slave = 6)
        time.sleep(0.2)
        getElect3LVX = client.read_holding_registers(800, 6, slave = 6)
        time.sleep(0.2)
        if gasType :
        ...
        if dryType :
            ...
            try :
                ...
                pass
        dataLVX = dataParser(exhibitStat, getTempLVX, getElect1LVX, getElect2LVX, getElect3LVX, 0, 0, dataLen, CTratio, PTratio)
        inputDataLVX = dataLVX[0:39]
        inputDataHV = HVParser(inputData, inputDataLVX, fixedTapRatio)
        ...
        if int((datetime.datetime.now() - excelPrevTime).total_seconds()) > 3:
            ...
            for i in range(0, 3):
                ...
                for row in sendHarm:
                    sheetHarm.append(row)
            #LVX & HV
            sendLVX = [datetime.datetime.now().strftime("%H:%M:%S")] + inputDataLVX
            sendLVX = ((tuple(sendLVX)),)
            sheetLVX = wb["LV_X"]
            for row in sendLVX:
                sheetLVX.append(row)
            sendHV = [datetime.datetime.now().strftime("%H:%M:%S")] + inputDataHV
            sendHV = ((tuple(sendHV)),)
            sheetHV = wb["HV"]
            for row in sendHV:
                sheetHV.append(row)           
            ...
            sheetMain = wb["Raw_data"]
            for row in sendLog:
                sheetMain.append(row)
